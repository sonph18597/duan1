...
<?php
    ...
    if(isset($_GET['ma_don_hang'])){
        ...
        $sql = "SELECT so_luong,ten_ca,gia_ban FROM chi_tiet_don_hang JOIN ca ON ca.ma_ca=chi_tiet_don_hang.ma_ca WHERE ma_don_hang='$ma_don_hang'";
        $stmt = $conn->prepare($sql);
        $stmt->execute();
        $list_ct_don_hang=$stmt->fetchAll();

        $sql = "SELECT tong_tien FROM don_hang WHERE ma_don_hang='$ma_don_hang'";
        $stmt = $conn->prepare($sql);
        $stmt->execute();
        $tong_tien=$stmt->fetch()[0];
    }
